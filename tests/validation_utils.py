...
def validate_converting_output(
    ...
    assert len(os.listdir(outputs_folder)) == len(
        enabled_scrapers
    ), f"Expected csv files per chain, found {len(os.listdir(outputs_folder))}"
    ...
    assert len(os.listdir(converting_status_folder)) == len(enabled_scrapers) * len(
        FileTypesFilters
    ), f"Expected {len(enabled_scrapers) * len(FileTypesFilters)} status files, found {len(os.listdir(converting_status_folder))}"
