...
class LongTermDatasetManager:
    ...
    def compose(self):
        ...
        increments the dataset version index, and packs staged files into
        one zip per scraper.
        """
    ...
    def reverse(self, dataset_path):
        ...
        if os.path.isdir(dataset_path):
            ...
            for file in os.listdir(dataset_path):
                ...
                if (
                    not any(
                        scraper.lower() in file.lower()
                        for scraper in self.enabled_scrapers.split(",")
                    )
                    and file != "parser-status.json"
                ):
                    continue
