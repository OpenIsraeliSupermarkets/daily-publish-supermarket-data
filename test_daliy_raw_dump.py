...
class TestSupermarketDataPublisher(unittest.TestCase):
    ...
    def test_setup_schedule(self, mock_schedule):
        ...
        for occasion in self.publisher.occasions:
            mock_schedule.every().day.at(occasion).do.assert_called_with(self.publisher.run_scraping)

    @patch('daliy_raw_dump.schedule')
    @patch('daliy_raw_dump.time.sleep', return_value=None)
    def test_execute_scraping(self, mock_sleep, mock_schedule):
        mock_schedule.run_pending.side_effect = lambda: setattr(self.publisher, 'executed_jobs', len(self.publisher.occasions))
    ...
    def test_upload_to_kaggle(self, MockKaggleDatasetManager):
        ...
        mock_manager.compose.assert_called_once_with(outputs_folder=self.publisher.outputs_folder, status_folder=self.publisher.status_folder)
    ...
    def test_run(self, mock_setup_schedule, mock_execute_scraping, mock_execute_converting, mock_upload_to_kaggle, mock_clean_folders):
        self.publisher.run()
